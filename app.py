...
if df is None:
    for file in os.listdir('.'):
        if (file.endswith('.csv') or file.endswith('.xlsx')) and file != 'app.py' and "Simulasi" not in file:
            if any(kunci in file for kunci in ["Formulir", "Responses", "Jawaban"]):
                try:
...
if df is not None:
    ...
    if menu == "📊 Ringkasan Eksekutif & Status Temuan":
        ...
        p1 = df_filtered['P1_Regulasi'].mean(); p2 = df_filtered['P2_Finansial'].mean(); p3 = df_filtered['P3_Integritas'].mean(); p4 = df_filtered['P4_Operasional'].mean(); p5 = df_filtered['P5_Reputasi'].mean()
        ...
        st.info("Asisten AI siap membantu Anda menganalisis temuan audit secara instan berbasis data.")
        ...
        if api_key_input:
            try:
                ...
                if user_query:
                    ...
                    Anda adalah SRIS Executive AI Consultant, seorang ahli sistem manajemen ISO 27001:2022 dan auditor senior.
                    Tugas Anda adalah menjawab pertanyaan pengguna secara akurat, profesional, dan solutif HANYA berdasarkan data hasil audit berikut ini:
                    ...
                    Aturan penting:
                    1. Jawablah menggunakan Bahasa Indonesia yang profesional ala konsultan manajemen terkemuka.
                    2. Jika data tidak memuat jawabannya, jawab dengan santun bahwa data tersebut tidak tersedia di laporan audit saat ini.
                    3. Berikan saran rencana perbaikan (CAPA) yang relevan jika diminta.
                    """
                    
                    with st.spinner("Mengonfirmasi data ke AI Engine..."):
                    ...
                    st.markdown(f"<div class='chat-bubble-bot'>🤖 <b>SRIS AI:</b> {bot_response}</div>", unsafe_allow_html=True)
